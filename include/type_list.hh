...
#ifndef LP_CC_LIB_TYPE_LIST_HH
#define LP_CC_LIB_TYPE_LIST_HH
...
namespace lp {
    ...
    struct get_item {
        static_assert(N < List::size, "Can't get item with index higher than size");
    ...
    template <template <typename ...> class Assign, typename ...Types>
    struct get_assign {
    ...
    template <template <typename> class Predicate, typename List, typename ...Types>
    ...
    struct type_list {
        ...
        template <template <typename> class Predicate>
        using filter = typename conditional_append<Predicate, type_list<>, Types...>::type;
